...
class TVector {
  ...
  TVector(double x, double y, double z, TStatus s)
      : _x(x)
      , _y(y)
      , _z(z)
      , _Status(s) { }
  ...
  static TVector& unit(const TVector& v, TVector& result) {
    result = v;
    return result.unit();
  }
  ...
  static TVector Default(const TVector& v, TVector& result) {
    result = v;
  ...
  TVector operator*(const double& scale) const {
    TVector tv;
